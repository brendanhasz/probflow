...
def test_accuracy():
    ...
    # Compare metric
    assert np.isclose(metrics.accuracy(y_true, pred_dist.mean()), 2.0 / 3.0)


def test_mean_squared_error():
    """Tests probflow.utils.metrics.mean_squared_error."""
    # Predictive dist
    preds = to_default_dtype([0, 1, 2, 0, 0, 0])
...
def test_sum_squared_error():
    ...
    # Compare metric
    assert np.isclose(
        metrics.sum_squared_error(y_true, pred_dist.mean()), 10.0
    )
...
def test_mean_absolute_error():
    ...
    # Compare metric
    assert np.isclose(
        metrics.mean_absolute_error(y_true, pred_dist.mean()), 1.0
    )
...
def test_r_squared():
    ...
    # Predictive dist
    preds = to_default_dtype([0, 1, 2, 2, 2])
    pred_dist = Normal(preds, 1)
    y_true = np.array([0, 1, 2, 3, 4]).astype("float32")

    # Compare metric
    assert np.isclose(metrics.r_squared(y_true, pred_dist.mean()), 0.5)


def test_true_positive_rate():
    """Tests probflow.utils.metrics.true_positive_rate."""
    # Predictive dist
    probs = to_default_dtype([1, 1, 1, 1, 1, 0])
    pred_dist = Bernoulli(probs=probs)
    y_true = np.array([1, 0, 1, 1, 0, 1]).astype("float32")

    # Compare metric
    assert np.isclose(
        metrics.true_positive_rate(y_true, pred_dist.mean()), 0.75
    )
...
def test_precision():
    ...
    # Compare metric
    assert np.isclose(metrics.precision(y_true, pred_dist.mean()), 3.0 / 5.0)


def test_f1_score():
    """Tests probflow.utils.metrics.f1_score."""
    # Predictive dist
    probs = to_default_dtype([0, 1, 1, 1, 1, 0])
    pred_dist = Bernoulli(probs=probs)
    y_true = np.array([1, 0, 1, 1, 0, 0]).astype("float32")
    ...
    assert np.isclose(metrics.f1_score(y_true, pred_dist.mean()), f1)
...
def test_get_metric_fn():
    ...
    tpr = 2 / 3
    f1 = 2 * (ppv * tpr) / (ppv + tpr)
    assert np.isclose(metric_fn(y_true, pred_dist.mean()), f1)
