"""
Parameters are values which characterize the behavior of a model.  When fitting
a model, we want to find the values of the parameters which best allow the
model to explain the data.  However, with Bayesian modeling we want not only to
find the single *best* value for each parameter, but a probability distribution
which describes how likely any given value of a parameter is to be the best or
true value.
...
of that distribution (the value of the parameters which define the
distribution) is optimized while fitting the model.  See the
:doc:`../user_guide/math` section for more info.
...
* :class:`.MultivariateNormalParameter` - parameter with a multivariate Normal
  posterior
* :class:`.CenteredParameter` - parameter array with a mean of 0

See the :doc:`user guide <../user_guide/parameters>` for more information on
Parameters.
